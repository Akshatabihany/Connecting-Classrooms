...
app.post("/chat.html",function(req,res){
	...
	res.render("chat",{"room":req.query.room,"chatstudent":req.query.username})
})
...
io.on('connection', (socket) => {
    ...
    // Chat.find({Room: 'JavaScript'}).sort('Time').exec((err, docs) => { 
	// 	if(err) console.log(err);
	// 	else
	// 	console.log(docs)
	// 	for(let i=0;i<docs.length;i++)
	// 	{
	// 	  io.to(user.room).emit('message', formatMessage(docs[i].sender, docs[i].msg));
	// 	}
	//    });
